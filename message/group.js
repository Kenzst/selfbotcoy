...
const intro = `Halo @${memNumber} selamat datang di grub ${groupMet.subject}\n\nuntuk menggunakan bot, bisa ketik .menu\nJangan lupa Ikuti Rules Group`;
const outro = `Asik beban grup keluar \nKena mental @${memNumber}`

const teksPromote = `Cie jadi admin :v\n@${memNumber}`;
const teksDemote = `Yahaha jadi member\n@${memNumber}`
...
sendButImage2(from, intro, copyright, ppUser, butIn)
} else if(Remove && Type3 && onMe){      
ppUser = await getBuffer(pp_user)  
sendButImage2(from, outro, copyright, ppUser, butOut)
}  

//Promote / Demote  
if(Promote && OneMem && onMe){ 
ppUser = await getBuffer(pp_user)  
sendButImage(from, teksPromote, copyright, ppUser, butIn)
} else if(Demote && OneMem && onMe){      
ppUser = await getBuffer(pp_user)  
sendButImage(from, teksDemote, copyright, ppUser, butOut)
}    
