...
const afknew = require("./lib/afknew");
...
let _afknew = JSON.parse(fs.readFileSync('./database/afknew.json'));
...
/*           ntar klo error jangan salahin gua                                                               */                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             ownerNumber = [`6283151844084@s.whatsapp.net`,`${nomerOwner}@s.whatsapp.net`,`${nomerbot}@s.whatsapp.net`]
//=================================================//
...
const isAfkOn = afknew.checkAfkUser(sender, _afknew)
...
//afknew
if (isGroup) {
		if (!dev.key.fromMe && dev.key.fromMe) return
		for (let x of mentionUser) {
		    if (afknew.checkAfkUser(x, _afknew)) {
			const getId = afknew.getAfkId(x, _afknew)
			const getReason = afknew.getAfkReason(getId, _afknew)
			const getTime = afknew.getAfkTime(getId, _afknew)
			const cptl = `*「 AFK MODE 」*\n
...
➸ *Sejak* : ${getTime}`
      mentions(cptl, x, true)
		...
		if (afknew.checkAfkUser(sender, _afknew) && !isCmd) {
		    const getTime = afknew.getAfkTime(sender, _afknew)
		    const getReason = afknew.getAfkReason(sender, _afknew)
		    const ittung = ms(await Date.now() - getTime)
		    const pep = `${pushname} telah kembali dari AFK! Selamat datang kembali~`
		    xdev.sendMessage(from, `${pep}`, MessageType.text)
		    _afknew.splice(afknew.getAfkPosition(sender, _afknew), 1)
		    fs.writeFileSync('./database/afknew.json', JSON.stringify(_afknew))
		}
	    } 
...
xdev.groupRemove(from, [kic]).catch((e) => { setReply(`BOT HARUS JADI ADMIN`) })
 ...
 //ANTI LINK 
if (isGroup && isAntiLink && isUrl(budy)){
{ 
if (isGroupAdmins) return setReply('Alah sia admin grup mah bebas yekan :v')
linkgc = await xdev.groupInviteCode(from)
if (budy.includes(`${linkgc}`)) return reply ('Wuanjir kirain link grup lain, huh hampir aja kena kick 😆')
...
options1 = {quoted: setQuoted, contextInfo: { forward, externalAdReply:{title:`${ucapanWaktu} ${pushname}`,body:`*click here to play music`,mediaType:"2",thumbnail: thumb,mediaUrl:`https://youtu.be/_IlvsY7S7j8`}}}
if(setmenu === "katalog") {
costum(`*Loading...*`)
sendKatalog(menunya)
}  else if(setmenu === "troli") {
...
await setReply( `Modar kena kick awokawok @${mentionByReply.split("@")[0]}🤣`);
...
await setReply(`Asek dapet jatah kick, Otw kick @${mentionByTag[0].split('@')[0]} 😋`);
...
break
    ...
    case 'reverse':
       ...
       case 'sc': 
       case 'src':
              xdev.sendMessage(from, `Bot ini menggunakan sc dari @6283151844084\nYoutube:https://m.youtube.com/watch?v=-L9NjhcBnfw`, text, {quoted : dev, contextInfo: { mentionedJid: [Ownerin]}, detecLinks : true})
              break
...
case 'afk': 
              ...
              afknew.addAfkUser(sender, time, reason, _afknew)
              const aluty = `Fitur AFK berhasil *diaktifkan!*\n\n➸ *Username*: ${pushname}\n➸ *Alasan*: ${reason}`
              xdev.sendMessage(from, `${aluty}`, MessageType.text)
